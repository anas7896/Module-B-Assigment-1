...
function updateModelDropdown() {
  ...
  if (brand) {
    for (const model in mobiles[brand]) {
      const option = document.createElement("option");
      option.value = model;
...
// Filter mobile based on selected brand and model
function filterMobile() {
  ...
  if (brand && model) {
    ...
    for (const key in mobile) {
      ...
      detail.textContent = `${key}: ${JSON.stringify(mobile[key])}`;
      mobileDetails.appendChild(detail);
    }
    resultDiv.appendChild(mobileDetails);
  } else {
